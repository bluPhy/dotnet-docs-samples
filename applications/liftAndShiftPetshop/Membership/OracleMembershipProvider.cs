...
namespace PetShop.Membership
{
    class OracleMembershipProvider : MembershipProvider
    {
		...
		public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (_applicationName.Length > 255)
                throw new ProviderException("Provider application name is too long, max length is 255.");

            // Get application ID
            _applicationId = GetApplicationID(_applicationName, true);
            if (_applicationId == 0)
		...
		public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object userId, out MembershipCreateStatus status)
        {
            ...
            if (_passwordStrengthRegularExpression != null)
            {
                if (!Regex.IsMatch(password, _passwordStrengthRegularExpression))
                {
                    status = MembershipCreateStatus.InvalidPassword;
                    return null;
                }
            ...
            {
                if (Regex.Matches(password, @"\W").Count < _minRequiredNonalphanumericCharacters)
                {
                    status = MembershipCreateStatus.InvalidPassword;
                    return null;
                }
            ...
            try
            {
                ...
                int uid = GetUserID(transaction, _applicationId, username, true, false, dt, out isUserNew);
                if (uid == 0)
                ...
                if (IsEmailInMembership(transaction, email, _applicationId))
                {
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            try
            {
                ...
                int uid = GetUserID(transaction, _applicationId, username, false, false, dt, out isUserNew);
                if (uid <= 0)
                    success = false;
